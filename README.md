...
1.生成指定长度的公钥和私钥参数(n,e,d,p,q);

2.读取PKCS#8格式的.pem文件;

3.PKCS#1 v1.5(1998年) rfc2313的PKCSv1.5-PKCS1加密解密、签名验签;

4.PKCS#1 v2.2(2016年) rfc8017的PKCSv2.2-OAEP加解密、RSASSA-PSS签名验签;

因为在实际项目中遇到RSA加解密的问题，网上找了很多资料都无法较好的了解RSA的详细过程，为了后来者能更全面的详细的学习RSA，因此创建了本项目。本项目主要用于学习RSA使用，通过本项目你可以学习到整个RSA加密过程中除哈希以外的所有步骤。由于该项目主要用于学习使用，所以各项功能完成的并不是十分完善，上述的所有功能仅测试过 256字节长度的私钥，其他长度应该也能用，不过没有测试。

该项目主要参考了 [RSA原理与Python3实现 - 0.说在最前面_哔哩哔哩_bilibili](https://www.bilibili.com/video/BV1VY411h7RB/?spm_id_from=333.788&vd_source=e35641d979879a7a423184450ee50336) 这个视频，如果您用的满意请为该up主投币点赞。

解析pem文件时主要参考了 https://github.com/xiangyuecn/RSA-csharp 项目中的内容，我把参考的部分已经截取到后面的说明中，侵权必删。

在使用中如果你遇到什么问题可以进入qq群910269729询问和反映，我看到了就会为大家解答，非常欢迎大家为该项目添加更多的内容。

如果需要进行代码讲解或者是解决项目问题也可以找我，不过需要付费0.0。
# 文件

 **RSA_learn.py** - 程序入口，定义了一个大类，组合了项目的所有功能，并提供了demo。

 
 **Auxiliary_function.py** - 为本项目提供了通用的辅助函数。

 
 **analysis_pem.py** -  解析pem格式的文件，文件中有demo用法，解析规则下面有说明。目前仅支持PKCS8格式的解析，PKCS1的可以去 http://tool.chacuo.net/cryptrsapkcs1pkcs8 该网址进行转换，这里仅支持解析私钥，因为该项目主要作为学习和私下测试使用，并不推荐直接用在项目中，所以公钥解析的部分就没有做。私钥中包含了公钥的内容，所以并不影响该项目的各个功能的使用。
 
**RSA_Key_Generation.py** - 生成一组RSA密钥对参数(n,e,d,p,q)，该文件中已经提供了demo用法。

**RSA_PKCS_v1_5.py** - RSA_PKCS_v1.5版本的加密解密、签名验签。

**RSA_PKCS_v2_2.py** - RSA_PKCS_v2.2版本的加密解密、签名验签。

**key_to_pem.py** - 这是一个单独的文件，并不影响项目的运行，如果你想通过(n,e,d,p,q)生成pem格式的文件，就使用它，在此之前你需要安装对应的库文件。
...
此部分为参考 https://github.com/xiangyuecn/RSA-csharp 项目中的部分，主要解释了如何解析PEM格式的文件，侵权必删，只是为了方便大家参考。
